...
If `~/.fabro/settings.toml` does not yet exist, `fabro server start` enters **install mode**: it prints an install URL and a one-time install token, attempts to open the URL in your default browser, and serves a web wizard that walks you through configuring your server URL, Azure sandbox platform settings, the shared object store, LLM provider, and GitHub integration.
...
The `Azure` step saves the non-secret sandbox platform settings under `[server.sandbox.azure.platform]` and stores optional ACR pull credentials in the vault instead of the config file.
