...
Optional `FABRO_AZURE_ACR_USERNAME` and `FABRO_AZURE_ACR_PASSWORD` do not live in
`settings.toml`. When provided through `fabro install`, Fabro stores them in the vault and
resolves them together for Azure sandbox image pulls.
...
Azure ACR credentials saved by the install wizard are different: Fabro stores
`FABRO_AZURE_ACR_USERNAME` / `FABRO_AZURE_ACR_PASSWORD` in the vault instead of `server.env`.
...
### Azure sandbox registry credentials (optional)

Fabro resolves these from the vault for the server-owned Azure sandbox platform config.

| Variable | Description |
|---|---|
| `FABRO_AZURE_ACR_USERNAME` | Optional ACR username used when pulling private sandbox images |
| `FABRO_AZURE_ACR_PASSWORD` | Matching ACR password |

The install wizard stores these only as a pair. If you configure them manually, keep both set or
both unset.
