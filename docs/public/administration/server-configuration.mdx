...
<Note>
  Fabro only reads `settings.toml`. Older `server.toml`, `user.toml`, and
  `cli.toml` filenames are no longer part of the supported config surface.
</Note>
...
| Scope                                                                       | Examples                                                                                                                                                                                                                                             |
| --------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Server-owned (runtime-only from local `settings.toml`)                      | `[server.listen]`, `[server.api]`, `[server.web]`, `[server.auth]`, `[server.storage]`, `[server.sandbox.azure.platform]`, `[server.artifacts]`, `[server.slatedb]`, `[server.scheduler]`, `[server.logging]`, `[server.integrations]`, `[features]` |
...
acr_identity_resource_id = "/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/fabro-prod/providers/Microsoft.ManagedIdentity/userAssignedIdentities/fabro-prod-sandbox-pull"
sandboxd_port = 7777
...
| Key              | Description                                      | Default   |
| ---------------- | ------------------------------------------------ | --------- |
| `provider`       | Object store backend: `local`, `s3`, or `azure`  | `"local"` |
...
`[server.artifacts]` together.

- `Local disk` uses `<storage_root>/objects`
- `AWS S3` uses one shared bucket with fixed prefixes for `artifacts`, `slatedb`, and `run-logs`
- `Azure Blob` uses one shared storage account container with fixed prefixes for `artifacts`, `slatedb`, and `run-logs`, plus runtime managed identity auth
...
The wizard covers `Local disk` with one shared on-host directory, `AWS S3` bucket/region with
either runtime credentials or manually-entered `AWS_ACCESS_KEY_ID` /
`AWS_SECRET_ACCESS_KEY`, and `Azure Blob` storage account + container using the server's runtime
managed identity.
...
honors those hand-edited values even though the browser wizard does not manage those advanced
S3-compatible options.

```toml title="Azure Blob object store"
[server.artifacts]
provider = "azure"
prefix = "artifacts"

[server.artifacts.azure]
account = "fkukuckfabrosbx01"
...
### `[server.sandbox.azure.platform]` section

Configure the server-owned Azure platform settings used when `run.sandbox.provider = "azure"`.

| Key                        | Description                                                                    | Default |
| -------------------------- | ------------------------------------------------------------------------------ | ------- |
| `subscription_id`          | Azure subscription used for sandbox provisioning                               | —       |
| `resource_group`           | Resource group that owns the ACI sandboxes                                     | —       |
| `location`                 | Azure region for sandboxes                                                     | —       |
| `subnet_id`                | Delegated subnet resource ID for Azure Container Instances                     | —       |
| `acr_server`               | ACR login server used by sandbox images                                        | —       |
| `acr_identity_resource_id` | User-assigned managed identity resource ID attached to sandboxes for ACR pulls | —       |
| `sandboxd_port`            | Port served by `fabro-sandboxd` inside the sandbox                             | `7777`  |

Fabro persists these non-secret values in `settings.toml`. The browser install wizard's `Azure`
step writes this section automatically.

For the supported Azure path in this repo, set `acr_identity_resource_id` to the Terraform output
`acr_identity_resource_id`. That value should be the dedicated low-privilege sandbox-pull identity,
not the privileged `fabro-server` identity. Fabro then uses that sandbox-attached managed identity
for private ACR pulls instead of separate registry credentials.

For Azure sandboxes, configure this section instead of relying on worker-local
`FABRO_AZURE_SUBSCRIPTION_ID`, `FABRO_AZURE_RESOURCE_GROUP`, `FABRO_AZURE_LOCATION`,
`FABRO_AZURE_SANDBOX_SUBNET_ID`, or `FABRO_AZURE_ACR_SERVER` environment variables.
...
For the supported Azure path in this repo, private sandbox image pulls use the configured
`acr_identity_resource_id`, so no separate Azure ACR username/password secrets are required.
