...
The Terraform environment already wires the Azure Files `/storage` mount, ingress, identity, and the Azure sandbox provider environment variables into the Container App. Azure sandboxes use ephemeral `emptyDir` storage for `/workspace`. Use immutable tags for both the server image and any sandbox images you build.
