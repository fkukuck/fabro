...
**"Azure platform config is not configured"** — Azure runs now depend on the server's persisted Azure platform settings, not ad hoc `FABRO_AZURE_*` worker env vars. Re-run `fabro install` or the browser install wizard, complete the `Azure` step, and restart the server. If you manage config manually, write the non-secret values under `[server.sandbox.azure.platform]` and only set `FABRO_AZURE_ACR_USERNAME` / `FABRO_AZURE_ACR_PASSWORD` together when your ACR requires them.
