...
| **Sandbox GITHUB_TOKEN** | When `[run.scm.github.permissions]` are declared in run config, Fabro mints a scoped Installation Access Token and injects it as `GITHUB_TOKEN` in the sandbox |
...
`server.integrations.github` remains the server host's owner-scoped configuration for strategy and credentials. Workflow and project files should request sandbox token scope through `run.scm.github.permissions`; `server.*` stanzas in those files are schema-valid but inert in remote-server mode.
