---
title: "GitHub"
description: "Integrate Arc with GitHub for repository access and OAuth login"
---

Arc uses a [GitHub App](https://docs.github.com/en/apps/overview) to authenticate users in the web UI and to clone private repositories into remote sandboxes. The GitHub App is created automatically through a guided setup flow — no manual app configuration required.
...
- The Arc web app running (`cd apps/arc-web && bun run dev`)
...
4. GitHub redirects back to Arc, which automatically:
   - Exchanges the temporary code for permanent app credentials
   - Writes `app_id`, `client_id`, and `slug` to `~/.arc/server.toml`
...
5. **Install the app** on your GitHub account or organization. Go to `https://github.com/settings/apps/{your-app-slug}/installations` and install it on the repositories Arc should access.
...
| Field | Source |
|---|---|
| `git.app_id` | `~/.arc/server.toml` |
| `git.client_id` | `~/.arc/server.toml` |
...
The private key is stored as base64-encoded PEM. Arc also accepts raw PEM format (starting with `-----BEGIN`).
...
2. Arc redirects to GitHub's OAuth authorization endpoint with scopes `read:user` and `user:email`
3. User authorizes the app on GitHub
4. GitHub redirects back with an authorization code
5. Arc exchanges the code for an access token and fetches the user's profile and verified email
6. Arc checks the username against the `allowed_usernames` list in `server.toml`
...
When a workflow runs in a remote sandbox (Daytona or Docker), Arc clones the current repository into the sandbox using the GitHub App:

1. Arc detects the local repository's `origin` remote URL and current branch
2. SSH URLs (e.g. `git@github.com:owner/repo.git`) are converted to HTTPS
3. Arc signs a short-lived JWT using the App ID and private key (RS256, 10-minute validity)
4. Using the JWT, Arc looks up the GitHub App installation for the repository (`GET /repos/{owner}/{repo}/installation`)
5. Arc requests a scoped Installation Access Token with `contents: write` permission on the specific repository
...
After each workflow stage, Arc [checkpoints](/execution/checkpoints) by pushing the run branch and metadata branch to origin. Inside remote sandboxes, the git remote URL is configured with the Installation Access Token for authenticated pushing.

For long-running workflows, Arc refreshes the token before each push since Installation Access Tokens are short-lived (typically 1 hour).
...
If you see `Git clone failed ... If this is a private repository, configure a GitHub App`, the GitHub App credentials are not configured. Run the setup flow through the web UI or verify with `arc doctor`.
